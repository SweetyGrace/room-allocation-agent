...
/**
 * Interface for daily registration data
 */
...
/**
 * Interface for video session data
 */
interface VideoData {
  absentees: number;
  attendees: number;
...
/**
 * Interface for non-video session data
 */
...
/**
 * Interface for combined data props
 */
interface DataProps {
  video: VideoData;
...
/**
 * Interface for LineChart component props
 */
...
/**
 * ReusableLineChart Component
 * Renders a line chart showing attendance or registration data with toggle functionality
 * between video and non-video sessions
 */
const ReusableLineChart: React.FC<LineChartProps> = ({ data, title }) => {
  // Initialize refs, state and hooks
  ...
  // Get current data based on video toggle
  const currentData = isVideoEnabled ? data.video : data.nonVideo;  
  
  /**
   * Extract time labels from attendance data
   * Handles both video and non-video cases
   */
  ...
  /**
   * Extract day labels from registration data
   * Handles both video and non-video cases
   */
  const dayLabels = isVideoEnabled
    ? Object.keys(data.video.registered || {})
    : Object.keys(data.nonVideo.registered || {});

  /**
   * Process attendance data for chart display
   * Returns array of attendance values based on selected mode
   */
  ...
  /**
   * Process registration data for chart display
   * Returns array of registration values based on selected mode
   */
  ...
  // const totalDisabledVideo = currentData.disabledVideo;
   ...
   * Controls appearance and behavior of the line chart
   */
  const options = {
    responsive: true,
